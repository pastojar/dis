...
\ctusetup{
    ...
    title-english = {Value of precipitation estimates \\ from commercial microwave links \\ for rainfall-runoff modelling \\ in small urban catchments},
...
The bulk of the research presented in this thesis has been originally published in peer-reviewed journal or conference papers over the course of the past few years. The thesis thus partially documents the progress made within the research area. Therefore, although valid when first published, some statements, especially regarding the current state of knowledge, are outdated from the current point of view. Thus, especially when chapters \ref{chap4} and \ref{chap5} are considered, the reader is kindly asked to perceive the presented research in the context of the time when it was first published.
...
We address the above mentioned knowledge gaps and assess the value of CML QPEs for rainfall-runoff modelling in a small (1.3~km\textsuperscript{2}) urban catchment. The rainfall-runoff modelling performance associated with the CML QPEs is evaluated by quantifying uncertainties in the rainfall-runoff modelling outputs. We do this while trying to reduce the systematic errors in the CML QPEs with the help of three various reference datasets. We believe that an equivalent of at least one of these datasets could be available to most urban hydrology professionals.
...
Reference datasets used to calibrate the WAA estimation models include the following:
\begin{itemize}
...
Rainfall data retrieved using the above observation layouts are propagated through a rainfall-runoff model, and the model outputs are evaluated against observed discharges. We decide to quantify uncertainties of the rainfall-runoff model predictions and employ the Bayesian uncertainty analysis framework of  \cite{kennedy2001bayesian} as formulated by \cite{reichert2012linking} and first used in an urban hydrology context by \cite{giudice2013improving}. More recently, this approach has been successfully applied by \cite{sikorskaValueDifferentPrecipitation2018} to analyze the value of different precipitation data for flood prediction in an alpine catchment. The basic principle of the method is to extend the deterministic rainfall-runoff model by a stochastic error model which explicitly accounts for systematic model errors, i.e. bias. More information on the principles or particular details of this uncertainty quantification method can be found in chapters \ref{delGiudTheor} and \ref{Pilot_UncAn} respectively.
...
First, we present precipitation estimates retrieved using all six evaluated rainfall observation layouts (chapter \ref{paperIIIRes1}). Next, the effect of various natures of rainfall data from the individual observation layouts on runoff predictions is briefly introduced using the deterministic rainfall-runoff model only (chapter \ref{Scatter}). Then, results based on a quantification of the rainfall-runoff modelling output uncertainty are presented. We pay special attention to events with high rainfall spatial variability (chapter \ref{Summary_mostvar}), as they are often associated with both a low representativeness of traditional rain gauge observations and with high rainfall intensities which are of special interest for many urban hydrology tasks, e.g. evaluation of urban stormwater systems. Lastly, an exhaustive summary for all events and layouts (chapter \ref{Summary}) is presented. 


\FloatBarrier
\subsection{Retrieved rainfall} \label{paperIIIRes1}

Precipitation estimates retrieved using all six evaluated rainfall observation layouts (chapter \ref{pIII_rainfall_scen}) are presented in Fig. \ref{fig:III_1}. Unsurprisingly, 1-min rain gauge data from three local gauges and from three municipal ones resemble each other much more than 60-min data from the single gauge at the WWTP. Compared to the former two, observations from the latter layout contain more data points with low (< 2 mm/h) rainfall intensities and less with high ones (> 5 mm/h ). Analogically, CML QPEs derived using the WAA model calibrated to the data from the WWTP gauge, compared to the other two CML QPE layouts which are more alike, tend to overestimate rainfall intensities below roughly 0.7 mm/h and underestimate higher ones. 


\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{figs/paperIII/rain_plots_edt.png}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{figs/paperIII/Scatter w stats.png}
...
Discharges predicted using the deterministic rainfall-runoff model only are summarized in Fig. \ref{fig:III_2}. The systematic errors in rainfall estimates associated with the 60-min records from the gauge at the WWTP, and with CML QPEs derived using the WAA model calibrated to the gauge data, have resulted in biased runoff volume predictions (relative volume error $dV$ -0.101 and -0.129 respectively). When using 1-min data from the three municipal rain gauges, or CML QPEs with the WAA model calibrated to 60-min data from these gauges, the resulting bias is notably lower ($dV$ -0.052 and 0.040 respectively). 

Compared to the 1-min rain gauge data, both CML QPE observation layouts seem to reduce both the amount of outliers associated with rainfall events with high spatial variability and the general point spread perpendicular to the line $x=y$. These improvements are reflected also in the respective values of the $S\!C\!C$ performance metric, which are higher for both CML QPE observation layouts (0.893 and 0.908) than for the 1-min records from the municipal rain gauges (0.868), suggesting better reproduction of runoff temporal dynamics by the CML QPEs. This is analyzed more closely in the next section.


\FloatBarrier
\subsection{Most-spatialy-variable rainfalls} \label{Summary_mostvar}

For a single rainfall-runoff event with a high spatial rainfall variability, we compare the uncertainty of rainfall-runoff modelling predictions obtained using 1-min data from the three nearest municipal rain gauges (Fig. \ref{fig:III_3}, top) and using QPEs derived from the best-located CMLs with the WAA model calibrated to the 60-min data from the same gauges (Fig. \ref{fig:III_3}, bottom). The discharge peak is considerably better reproduced, in terms of both amplitude and timing, when using the CML QPEs. In fact, all modelling performance metrics we quantify reach, in general, better values, in terms of their medians, for the CML QPEs. However, it should be noted that the performance of median discharge  predictions (in purple) in terms of $S\!C\!C$, reflecting the temporal dynamics, is remarkable in both cases, only slightly higher for the CML QPEs (0.91 vs. 0.87). On the other hand, when the overall uncertainty bound prediction ($N\!M\!I\!S\!S$) is  considered and compared with other events, it is above-median for the CML QPEs (0.74), but quite inferior for the rain gauge data (0.53). 
...
The aptitude of CML QPEs is confirmed in Fig. \ref{fig:III_5} where, using data from eight events with the highest rainfall spatial variability, we summarize the rainfall-runoff modelling performance for all CML QPE observation layouts and make comparison with the performance of 1-min records from the three local rain gauges. The best-case-scenario rain gauge data outperform the CML QPEs in terms of $S\!C\!C$, both for medians (0.72 vs. 0.63--0.667) and variability. Systematic errors in CML QPEs with the WAA model calibrated to records from the rain gauge at the WWTP affect the relative error of maximal discharges $dQ_{max}$, which is -0.292 for this observation layout. However, for the  two other CML QPE layouts, $dQ_{max}$ is, in absolute values, lower (-0.127 for calibration to three municipal gauges; -0.103 for calibration to observed discharges) than for the rain gauge records (0.167). The overall uncertainty bound performance in terms of $N\!M\!I\!S\!S$ is also very similar for these three layouts, reaching practically the same medians of roughly 0.7 and varying between 0.5 and 0.85. Moreover, the rain gauge observations lead to a lower prediction reliability, only 0.95 in median, compared to the medians of circa 0.97 for the CML QPEs.



\FloatBarrier
\subsection{Summary of all events and all observation layouts} \label{Summary}
...
Nonetheless, the relevance of our findings for other small urban catchments should be subject to further research. Firstly, based on our previous study where we had analyzed several WAA models \cite[][chapter \ref{chap6}]{pastorekPrecipitationEstimatesCommercial2022}, we have tested only the WAA model originally derived by using one of our 16 CMLs and data from one of the three summer seasons investigated herein \citep{valtrExcessAttenuationCaused2019}. Moreover, all CMLs used in this study are from the same product family of the same manufacturer (Ericsson, Mini-Link) and have aged similarly due to exposure to similar climatic conditions. Nevertheless, identifying optimal WAA models might be more complicated, especially when the reference data are available only in low resolutions, for CML networks in other areas, potentially consisting of antennas of different hardware characteristics and exposed to different climates for different time periods. 



\section{Conclusions}

In this study, we have assessed the value of quantitative precipitation estimates (QPEs) retrieved from commercial microwave links (CMLs) for rainfall-runoff modelling in a small urban catchment by robustly quantifying model output uncertainties associated both with the CML QPEs and with traditional rain gauge data. Moreover, we have analyzed the possibilities of reducing systematic errors in the CML QPEs with the help of three various reference datasets, equivalents of which might be commonly available to urban hydrology professionals.

It has been shown that, when predicting rainfall runoff with CML QPEs derived using a wet-antenna attenuation (WAA) model calibrated to 60-min records from a single rain gauge at an 8-km distance from the catchment of interest, the performance of these CML QPEs has, in general, represented a notable improvement compared to using only the 60-min gauge data themselves. In fact, apart from considerable bias in the predicted discharges which have been underestimated by 8.5\% in median, and by up to 30\% for runoff maxima, such CML QPEs have been an improvement also compared to records from the three closest rain gauges from the municipal rainfall monitoring network. These results have confirmed the high potential of CML QPEs for rainfall-runoff modelling in small urban catchments where traditional long-term rainfall observations are not available in resolutions suitable for urban hydrology purposes.

We have also shown that, when calibrating the WAA model to both discharges observed at the catchment's outlet and to 60-min records from the three closest municipal rain gauges, virtually unbiased CML QPEs can be retrieved. These CML QPEs can then reproduce observed discharges almost as well as three local rain gauges installed around the studied catchment for research purposes. In fact, for rainfall events of high spatial variability, runoff maxima have been biased to a lesser extent when using the CML QPEs. This shows that CML QPEs can be successfully used for rainfall-runoff modelling also when no reference rainfall data, only runoff observations, are available. 
...
This thesis has aimed, in general, to robustly evaluate the potential of quantitative precipitation estimates (QPEs) obtained from commercial microwave links (CMLs) for rainfall-runoff modelling in small urban catchments. Furthermore, it has investigated which factors (e.g. position relative to the catchment, liability to bias) most affect the suitability of individual CMLs to be used as a source of rainfall data for rainfall-runoff modeling. In order to reach the highest quality of CML QPEs, possibilities of reducing the bias common in CML QPE retrieval have also been addressed. Moreover, as CML QPEs could be most useful in areas where the availability of traditional rainfall observations is limited \citep{gossetImprovingRainfallMeasurement2016}, a special attention has been payed to CML QPE retrieval in such data-scarce conditions.
...
The above results have suggested that CML QPEs corrected for systematic errors, e.g. by adjusting to gauges from a low-density network (roughly 1 gauge per 20–-25 km\textsuperscript{2}), could represent a relevant rainfall data source for urban hydrology, if traditional rain gauge data in high spatial resolutions (roughly  1~gauge per 0.5--1 km\textsuperscript{2}) are not available in the given catchment. Their value could be especially pronounced for monitoring of heavy rainfalls, which are crucial for design and operation of urban drainage systems. Similar conclusions have been later made by \cite{stranskyRunoffPredictionUsing2018} and \cite{dischImpactDifferentSources2019}. However, none of these studies has provided clues regarding the usage of CML QPEs in situations when adjusting to other rainfall data is not an option. Moreover, at the time of the study, we were able to use data only from three consecutive months covering 15 relevant (rainfall depth over 2 mm) rainfall-runoff events. Similarly, QPEs from only four out of 19 CMLs were available as both unadjusted and adjusted to rain gauge observations. Furthermore, differences among individual CMLs have not been addressed, only the mean of all available CMLs has been examined. These additional spaces for improvement have foreshadowed the direction of the subsequent research endeavors.


In chapter \ref{chap5}, an extensive dataset covering a three-year period, used also in the next chapters, has been leveraged for the first time. The study presented in this chapter has addressed the combined effect of the CML characteristics (path length, transmission frequency) and their location relative to the catchment on the efficient use of CML QPEs for hydrological applications. In particular, it has investigated ability of individual CMLs to provide relevant QPEs for urban rainfall-runoff modelling in a small urban catchment.

The results have demonstrated that all CML data pre-processing, characteristics of the individual CMLs, and their relative position influence the quality of the retrieved QPEs. Firstly, the position of a CML within the small urban catchment has been proven to affect the CML's ability to capture rainfall-runoff dynamics, as shorter CMLs with paths within or close to the catchment boundaries have reproduced runoff dynamics better, especially for heavy rainfalls. Interestingly, in terms of capturing the runoff dynamics, mean QPEs of all CMLs have outperformed QPEs from individual CMLs. However, it has also been shown that the sensitivity of CMLs to rainfall, given by their frequency, polarization, and length, is the most influential factor affecting the bias in CML QPEs and subsequent runoff, and that the ability of biased CML QPEs to provide reliable flow estimates is predominantly low. As expected, continuously adjusting the CML QPEs to data from rain gauges has reduced the bias to minimal values for all CMLs. Unfortunately, the adjustment has also considerably worsened the ability of CML QPEs to reproduce runoff dynamics during heavy rainfalls. In sum, this study has confirmed the potential of CML QPEs for rainfall runoff modelling in small urban catchments. However, it has also pointed out that, to make the best use of these innovative rainfall data, the possibilities of reducing their bias without compromising their ability to reproduce rainfall-runoff dynamics should be better investigated.


The study presented within chapter \ref{chap6} has analyzed how, when deriving QPEs from CMLs, wet antenna attenuation (WAA), a major source of the bias in CML QPEs, can be reliably estimated without dedicated rainfall monitoring. In particular, various WAA estimation models, including a newly proposed one, based on considerably different assumptions have been tested. Next, we have analyzed which of the studied models can provide reliable WAA estimates without being calibrated for each individual CML.

The results have shown that virtually unbiased QPEs could be retrieved from CMLs without the need for dedicated rainfall monitoring campaigns. CML QPEs with a bias lower than 5\% and $R\!M\!S\!E$ of 1 mm/h in the median have been obtained when the empirical models for WAA estimation have been calibrated to rainfall data from the permanent municipal rain gauge network with a spatial resolution of one gauge per 20\,--\,25 km\textsuperscript{2} and a temporal resolution of 15 minutes. It has also been shown that such high-quality QPEs can even be derived from short, sub-kilometer CMLs which are in general very prone to bias. Models relating WAA to rainfall intensity, implicitly or explicitly, have led to the best results. For the latter, parameter sets have been found to be suitable for CMLs of various path lengths operating at various frequency bands, which could thus be transferred to other locations with CMLs of similar antenna hardware characteristics. Moreover, it has been demonstrated how these WAA models can be successfully applied without any auxiliary rainfall observations, i.e. using CML data only. 

In sum, this study has confirmed both the potential of CMLs as a source of high-quality rainfall data and the importance of appropriate WAA correction when deriving the QPEs. The presented advances in minimizing the requirements on auxiliary data necessary, both during the calibration of WAA models and during their implementation in the CML data processing routine, represent a legitimate step towards the retrieval of reliable QPEs from large CML networks in conditions where rainfall data are scarce.


In chapter \ref{chap7}, having in mind that an extensive quantification of the uncertainties related to applying CML QPEs in urban hydrology, and a comparison with traditional rainfall data uncertainty, has not been presented yet in relevant literature, we have assessed the value of state-of-the-art CML QPEs for rainfall-runoff modelling in a small urban catchment by robustly quantifying model output uncertainties associated both with the CML QPEs and with traditional rain gauge data. Moreover, we have leveraged the above research and explored the possibilities to calibrate WAA estimation models using data that could be commonly available to many urban hydrology specialists.

It has been shown that, when predicting rainfall runoff with CML QPEs derived using a WAA model calibrated to 60-min records from a single rain gauge at an 8-km distance from the catchment of interest, the performance of these CML QPEs has, in general, represented a notable improvement compared to using only the 60-min gauge data themselves. In fact, apart from considerable bias in the predicted discharges which have been underestimated by 8.5\% in median, and by up to 30\% for runoff maxima, such CML QPEs have been an improvement also compared to records from the three closest rain gauges from the municipal rainfall monitoring network. These results have confirmed the high potential of CML QPEs for rainfall-runoff modelling in small urban catchments where traditional long-term rainfall observations are not available in resolutions suitable for urban hydrology purposes.
...
The following material presents additional information on  rainfall event characteristics (Table \ref{tab:App_1}) and CML data availability (Fig. \ref{fig:App_1}). Although the underlying observations are the same (see chapter \ref{DatRet}), the exact number of rainfall (or rainfall-runoff) events differs for each study presented in the thesis due to various availability of data from various devices used, or differences in the event definition. The following material is based on rainfall-runoff events as used for the study presented in chapter \ref{chap5} (see \ref{paperIdatAval}).
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=14cm]{figs/App/Fig S2.png}
