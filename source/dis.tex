...
The prediction reliability $reliab$~[-] is quantified as the fraction of discharge observations falling into the predicted 90\% confidence intervals. We also employ common summary hydrological metrics such as the total discharged volume during a whole given event  or the discharged volume during peak flow period.  In particular, we compute relative errors of the total discharged volume  $dV$ [-] and of the volume during peak flow $dV_{peak}$ [-] for every single prediction of the extended model. When calculating $dV_{peak}$, the time step with maximal discharge observed is identified first and the volume discharged during eight minutes around the time step (four minutes before and four after) is computed afterwards. The difference between the observed time with maximal discharge and the modelled one $dt_{Qmax}$ [h] is another metric we quantify for every model prediction.

To summarize the above metrics $dV$, $dV_{peak}$, and $dt_{Qmax}$ for the whole set of predictions associated with the given event and rainfall observation layout, we calculate their means and standard deviations. This is in accordance with \cite{fencl2013assessing} who also used the relative error of the total discharged volume and interpreted its mean $\overline{dV}$ as the metric's bias and its standard deviation $sd(dV)$ as its uncertainty.
