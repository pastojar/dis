...
\begin{figure}[h]
\begin{center}
\includegraphics[width=12.2cm]{figs/paperIII/Scatter w stats.png}
...
\caption{Hydrographs showing 90\% prediction bounds for a chosen rainfall-runoff event (2014-10-16, 15:32) obtained using 1-min data from the three municipal rain gauges (top) and CML QPEs retrieved with the WAA model calibrated using 60-min data from the same gauges (bottom). The violin-plots show the metrics ($dV, dQ_{max}, N\!N\!S\!E, S\!C\!C$) which can be evaluated for each individual model prediction within the uncertainty ensemble. The other two metrics (reliability, $N\!M\!I\!S\!S$) show the only value associated with the given event (in black) whereas the background boxplots (in light gray) show the variability among all 23 events used in the validation stage.} 
\label{fig:III_3_b} 
\end{center}
\end{figure}
...
If comparing the same two rainfall observation layouts for another event (Fig. \ref{fig:III_3_b}), the aptitude of CML QPEs is further confirmed. Although the volumes are considerably underestimated and the hydrograph rising limb is delayed, the municipal rain gauges have not registered practically any precipitation at all. Hydrographs obtained with these two rainfall observation layouts for the remaining six events with most spatially variable rainfall are presented in Appendix (Figs. \ref{fig:App_2} and \ref{fig:App_3}). In all six cases, the CML QPEs lead to the same- or better-quality overall rainfall-runoff modelling performance.

The rainfall-runoff modelling performance for all eight events with the highest rainfall spatial variability is summarized in Fig. \ref{fig:III_5}. Moreover, the performance of all CML QPE observation layouts is compared with the performance of 1-min records from the three local rain gauges. The best-case-scenario rain gauge data outperform the CML QPEs in terms of $S\!C\!C$, both for medians (0.72 vs. 0.63--0.667) and variability. Systematic errors in CML QPEs with the WAA model calibrated to records from the rain gauge at the WWTP affect the relative error of maximal discharges $dQ_{max}$, which is -0.292 for this observation layout. However, for the  two other CML QPE layouts, $dQ_{max}$ is, in absolute values, lower (-0.127 for calibration to three municipal gauges; -0.103 for calibration to observed discharges) than for the rain gauge records (0.167). The overall uncertainty bound performance in terms of $N\!M\!I\!S\!S$ is also very similar for these three layouts, reaching practically the same medians of roughly 0.7 and varying between 0.5 and 0.85. Moreover, the rain gauge observations lead to a lower prediction reliability, only 0.95 in median, compared to the medians of circa 0.97 for the CML QPEs.
...
\subsubsection{Rainfall from chapter 6} 
...
\subsubsection{Rainfall events and hydrographs from chapter 9} 
...
\begin{table}[h]
\begin{center}
\includegraphics[width=12cm]{figs/App/rains overview table paperIII.png}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=14cm]{figs/App/hydro6_remRGs_1min.png}
\caption{Hydrographs showing 90\% prediction bounds for six events with high spatial rainfall variability obtained using 1-min data from the three municipal rain gauges, not presented in the main text.} 
\label{fig:App_2}
\end{center}
\end{figure}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=14cm]{figs/App/hydro6_CMLQPEs_remRGs.png}
\caption{Hydrographs showing 90\% prediction bounds for six events with high spatial rainfall variability obtained using CML QPEs retrieved with the WAA model calibrated using 60-min data from the three municipal rain gauges, not presented in the main text.} 
\label{fig:App_3}
\end{center}
\end{figure}
