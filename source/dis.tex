...
\ctusetup{
    ...
    title-english = {Experimental evaluation \\ of precipitation estimates \\ from commercial microwave links \\ for rainfall-runoff modelling \\ in a small urban catchment},
...
\chapter{Assesing CML QPEs by quantifying uncertainties in rainfall-runoff predictions in a small urban catchment: The~pilot~study} \label{chap4}
...
\chapter{Assesing CML QPEs by quantifying uncertainties in rainfall-runoff predictions in a small urban catchment: The~final~study} \label{chap7}

%On the value of CML QPEs for urban rainfall-runoff modelling: The final study

\rule{\textwidth}{0.4pt}
blabla
bla bla         DOPLNIT ABSTRAKT !
\newline
\rule[0.2cm]{\textwidth}{0.4pt}
{\footnotesize The bulk of this chapter represents a paper manuscript which is about to be submitted to be considered for publication in ... }
...
\subsection{Stochastic predictions: Most-spatialy-variable rainfalls} \label{Summary_mostvar}
...
If comparing the same two rainfall observation layouts for another event (Fig. \ref{fig:III_3_b}), the aptitude of CML QPEs for rainfall-runoff model predictions is further confirmed. Although the volumes are considerably underestimated and the hydrograph rising limb is delayed when using the CML QPEs, the municipal rain gauges have actually not registered practically any precipitation at all. Hydrographs obtained with these two rainfall observation layouts for the remaining six events with the most spatially variable rainfall are presented in Appendix (Figs. \ref{fig:App_2} and \ref{fig:App_3}). In all six cases, the CML QPEs lead to the same- or better-quality overall rainfall-runoff modelling performance.
...
As expected, continuously adjusting the CML QPEs to data from rain gauges has reduced the bias to minimal values for all CMLs. Unfortunately, the adjustment has also considerably worsened the ability of CML QPEs to reproduce runoff dynamics during heavy rainfalls. This is likely because the adjustment method strongly depends on the rain gauge data reliability, but the gauges are too far from each other to accurately observe small-scale rainfall variability. Moreover, when deriving the CML QPEs using this approach, the rainfall retrieval parameters change in 15-min time steps, which might be too coarse to reflect the underlying physical processes. Thus, although the adjusting is conceptually promising for eliminating the bias, it requires further development.
...
\chapterstarmark{Author's publications}
